(() => {
  ...
  function setFilterOptions() {
    ...
    all.value = "all";
    all.textContent = "All Quizzes Combined";
  ...
  async function fetchResults() {
    ...
    if (!db) {
      ...
      return ids.length ? rows.filter(row => ids.includes(row.quiz_id)) : rows;
    }

    let query = db
      .from("quiz_results_public")
      .select("id, student_id, learner_name, quiz_id, quiz_title, score, total, percentage, created_at")
  ...
  function setStats(rows, learnersCount) {
    ...
    document.getElementById("quizCount").textContent = String(quizList.length);
    document.getElementById("classAverage").textContent = rows.length
      ? `${Math.round(rows.reduce((sum, row) => sum + Number(row.percentage || 0), 0) / rows.length)}%`
      : "0%";
    document.getElementById("lastUpdated").textContent =
      new Intl.DateTimeFormat("en-ZA", { timeStyle: "short" }).format(new Date());
  }
  ...
  function renderAllQuizzes(rows) {
    const learners = groupByLearner(rows).map(learner => {
      ...
      const breakdown = bestPerQuiz
        .sort((a, b) => a.title.localeCompare(b.title))
        .map(item => `${item.title}: ${item.best}%`)
        .join(" • ");
    ...
    learners.forEach((learner, index) => {
      ...
      const cells = [
        ...
        `${learner.quizzes_done}/${quizList.length || learner.quizzes_done}`,
        "AVERAGE_CELL",
        "BEST_CELL",
        String(learner.attempts),
        `${learner.latest_quiz} (${formatDate(learner.latest_date)})`,
        learner.breakdown
      ];

      cells.forEach(value => {
        const td = document.createElement("td");
        if (value === "AVERAGE_CELL") td.appendChild(scoreCell(learner.overallAverage));
        else if (value === "BEST_CELL") td.appendChild(scoreCell(learner.bestScore));
        else {
          td.textContent = value;
          if (String(value).includes(" • ")) td.className = "quiz-breakdown";
        }
        tr.appendChild(td);
      });
      body.appendChild(tr);
  ...
  function renderSingleQuiz(rows, quizId) {
    ...
    learners.forEach((learner, index) => {
      ...
      const cells = [
        ...
        "AVERAGE_CELL",
        String(learner.attempts),
        formatDate(learner.last_attempt)
      ];

      cells.forEach(value => {
        const td = document.createElement("td");
        if (value === "LATEST_CELL") td.appendChild(scoreCell(learner.latest));
        else if (value === "BEST_CELL") td.appendChild(scoreCell(learner.best));
        else if (value === "AVERAGE_CELL") td.appendChild(scoreCell(learner.average));
  ...
  function render(rows) {
    ...
    let learnersCount = 0;
    if (selected === "all") learnersCount = renderAllQuizzes(rows);
    else learnersCount = renderSingleQuiz(rows, selected);
  ...
  async function loadResults() {
    try {
      ...
      errorMessage.textContent =
        "Results could not be loaded. Check config.js, the student-dropdown setup.sql, and student names/codes.";
  ...
  const seconds = Math.max(5, Number(config.RESULTS_REFRESH_SECONDS) || 10);
  window.setInterval(loadResults, seconds * 1000);

  if (db) {
    db.channel("dynamic-history-results")
      .on("postgres_changes", { event: "*", schema: "public", table: "quiz_results" }, loadResults)
      .on("postgres_changes", { event: "*", schema: "public", table: "quiz_registry" }, loadResults)
      .subscribe();
  }
})();
