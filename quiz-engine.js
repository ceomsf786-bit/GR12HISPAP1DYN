(() => {
  ...
  function loadQuizApp() {
    ...
    // Backward-compatible fallback for older quiz pages.
    const quizId = document.body.dataset.quizId;
    const quizMeta = (window.QUIZ_LIST || []).find(q => q.id === quizId) || {};
    ...
    return {
      ...
      group: config.DASHBOARD_GROUP || "history-live-quizzes",
  ...
  const groupId = String(app.group || config.DASHBOARD_GROUP || "history-live-quizzes").trim();
  ...
  function saveRegistryLocally(registryRow) {
    const rows = localJson(LOCAL_REGISTRY_KEY, []);
    const filtered = rows.filter(item => item.quiz_id !== registryRow.quiz_id);
    filtered.push(registryRow);
  ...
  function ensureLearnerSelect() {
    ...
    select.setAttribute("aria-label", "Select learner name");
    existing.replaceWith(select);

    const learnerPanel = select.closest(".learner-panel");
    const note = learnerPanel?.querySelector(".small-note");
    if (note) {
      note.textContent = "Select your official name, then enter the private learner code given by the teacher.";
    }
    return select;
  }
  ...
  function localStudents() {
    ...
    const raw = configured.length ? configured : saved;
    return raw.map((student, index) => ({
      ...
      class_group: student.class_group || student.classGroup || "Grade 12 History"
    })).filter(student => student.student_name && student.student_id);
  }
  ...
  async function fetchStudents() {
    ...
    const { data, error } = await db
      ...
      .select("student_id, student_name, class_group")
  ...
  function populateStudentsSelect(select, rows) {
    select.replaceChildren();

    const placeholder = document.createElement("option");
    placeholder.value = "";
    placeholder.textContent = rows.length ? "Select your name" : "No learners loaded yet";
    select.appendChild(placeholder);

    rows.forEach(student => {
      const option = document.createElement("option");
      option.value = student.student_id;
      option.textContent = student.class_group
        ? `${student.student_name} — ${student.class_group}`
  ...
  async function setupStudentDropdown() {
    ...
    select.innerHTML = `<option value="">Loading learner names…</option>`;

    try {
      students = await fetchStudents();
      studentsLoaded = true;
      populateStudentsSelect(select, students);
      if (!students.length && formMessage) {
        formMessage.textContent = "No learner names found. Add learners in Supabase first.";
      }
    } catch (error) {
      console.error(error);
      studentsLoaded = false;
      select.innerHTML = `<option value="">Learner names could not load</option>`;
      if (formMessage) {
        formMessage.textContent = "Learner list could not load. Check setup.sql and Supabase.";
      }
    }
  }
  ...
  async function validateStudentSelection() {
    ...
    if (!studentsLoaded) {
      return { ok: false, message: "Learner list is still loading. Wait a few seconds and try again." };
    }

    if (!studentId) {
      return { ok: false, message: "Please select your name from the list.", focus: select };
    }

    if (typedCode.length < 2) {
      return { ok: false, message: "Please enter your learner code.", focus: codeInput };
    }

    if (!db) {
      const match = students.find(student =>
        student.student_id === studentId && normaliseCode(student.student_code) === normaliseCode(typedCode)
      );
      if (!match) {
        return { ok: false, message: "That learner code does not match the selected name.", focus: codeInput };
      }
      return {
        ok: true,
        student_id: match.student_id,
        learner_name: match.student_name,
        learner_code: normaliseCode(typedCode)
      };
    }

    const { data, error } = await db.rpc("validate_quiz_student", {
  ...
  async function registerQuiz() {
    ...
    const { error } = await db
      .from("quiz_registry")
      .upsert(registryRow, { onConflict: "quiz_id" });

    if (error) {
      console.warn("Quiz registry could not be updated. Run the upgraded setup.sql once.", error);
    }
  }
  ...
  function applyMetaToPage() {
    ...
    if (descriptionEl) {
      descriptionEl.textContent = app.description || "30 random questions each attempt • Self-marking • Live saved results";
    }
  }

  function setModeBadge() {
    if (db) {
      modeBadge.textContent = "Live Supabase results";
      modeBadge.classList.add("live");
    } else {
      modeBadge.textContent = "Demo mode: this browser only";
      modeBadge.classList.remove("live");
    }
    document.getElementById("questionCountBadge").textContent =
      `${questionsPerAttempt} of ${fullBank.length} questions`;
  }
  ...
  function updateProgress() {
    ...
    progressText.textContent = `${answered} of ${attemptQuestions.length} answered`;
    progressBar.style.width = `${attemptQuestions.length ? Math.round((answered / attemptQuestions.length) * 100) : 0}%`;
  }
  ...
  function makeQuestionCard(question, index) {
    ...
    if (question.sourceText) {
      ...
      sourceLabel.textContent = question.sourceType
  ...
  function showResult(score, answers, savedLive) {
    ...
    document.getElementById("resultComment").textContent =
      `${comment} Result saved ${savedLive ? "to the shared live dashboard" : "in demo mode on this browser"}.`;
    ...
    attemptQuestions.forEach((question, index) => {
      ...
      answerLine.textContent = correct
        ...
        : `Your answer: ${answers[index] || "No answer"} • Correct answer: ${question.answer}`;
  ...
  form.addEventListener("submit", async (event) => {
    ...
    let learner;
    try {
      learner = await validateStudentSelection();
    } catch (error) {
      console.error(error);
      formMessage.textContent = "Learner code could not be checked. Confirm you ran the new setup.sql.";
      return;
    }

    if (!learner.ok) {
      formMessage.textContent = learner.message;
      learner.focus?.focus();
      return;
    }
    ...
    if (unansweredIndex !== -1) {
      ...
      document.querySelector(`[data-index="${unansweredIndex}"]`)
        .scrollIntoView({ behavior: "smooth", block: "center" });
      return;
    }

    const score = answers.reduce((sum, selected, index) => {
      return sum + (isCorrect(attemptQuestions[index], selected) ? 1 : 0);
    }, 0);
